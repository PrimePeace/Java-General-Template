...
这里推两个学习lombok的网址：
官网：Project Lombok
中文（推荐）：Java 插件Lombok用法 | Baeldung中文网
4.hutool 糊涂工具库（Hutool）
5.knife4j接口文档（Knife4j · 集Swagger2及OpenAPI3为一体的增强解决方案. | Knife4j）
6.mybatis-flex（MyBatis-Flex - MyBatis-Flex 官方网站）
7. 切面编程AOP（微服务——SpringBoot使用归纳——Spring Boot中的切面AOP处理——Spring Boot 中的 AOP 处理-阿里云开发者社区）
...
// 设置生成时间和字符串为空，避免多余的代码改动，和作者名定义
        ...
        return globalConfig;
启动类Been注解添加
@MapperScan("com.example.yuaicodemother.mapper")
yml基础配置
spring:
  application:
    name: 项目名
...
knife4j:
  ...
  setting:
    language: zh_cn
执行
修改代码生成器，运行一次MyBatisCodeGenerator 类通过访问 http://localhost:8123/api/doc.html#/home查看knife4j接口测试，如果能看到如图的  user-controller  表示成功，调试人员可以根据，实现服务方法，根据业务逻辑测试接口稳定性。
[图片]
注意
